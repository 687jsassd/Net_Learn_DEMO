...
## 关于半包问题
目前服务端和客户端的网络层，均实现了粘包处理，但均未实现半包处理。
在球数约达700时，此时一次广播时的数据包大小如下：
3字节包头+6*700字节坐标数据=4263字节，超过了客户端代码`data = cilent_socket.recv(4096)` 的4096字节接收缓冲区大小。
因为没有半包处理，所以会导致客户端接收不完整，从而导致数据解析错误，直接导致严重问题。
...
学习参考资料：
 - [TCP性能调优：深入理解发送与接收缓冲区的作用及配置](https://blog.csdn.net/m0_74670510/article/details/154698298)
 - [如何合理配置TCP缓冲区大小](https://ask.csdn.net/questions/9069777)
关于缓冲区大小,开大缓冲区会增加内存占用，但合理配置时对现代计算机影响很小
例如16384字节16KB的缓冲区，客户端影响根本没有，服务端假设处理1000连接，每个连接16KB缓冲区，共16MB内存占用，基本可以忽略不计。

在目前的设计中，唯一的大数据包出现在服务端向客户端的全量广播，
因此可以给客户端开16384字节的缓冲区。
服务端使用的是读消息头+再分配固定为消息体长度的切片的策略来进行正确的数据解析，因为已知消息体长度且动态分配切片，所以不会出现半包问题。
...
```
                if len(recv_buffer) < full_packet_len:
                    continue
```
这样的校验，可以确保缓存区中的数据是完整的。

注: 
- recv()函数是应用层每次从内核拷贝数据的最大长度，不是操作系统内核的 TCP 接收缓冲区（由系统参数配置，管理网络底层数据）
...
```
            ...
            if not data:
                ...
                connected = False
```
我们可以仅凭借if not data 来判断连接是否断开，原因如下：
RFC 793（TCP 核心国际标准）明确规定的行为：
主动关闭方发送FIN挥手包后，被动关闭方接收完所有数据，读操作必须返回 0 长度数据，表示连接已关闭。

而异常断开不会返回空数据，只会报错或阻塞；
| 对端连接状态 | 本地 recv(Python) | 本地 Read(Go) |
|---|---|---|
| 正常关闭（调用close，发送FIN包） | 空字节 b'' | 返回 n=0 |
| 异常断开（断网 / 崩溃，无FIN） | B | 抛重置错误 |


## 关于ID重用问题
之前提到，ID目前没有重用机制，会持续递增。
目前，服务端的ID实现如下：
```
var(
...
```
一种直观的方法是利用栈:
维护一个默认有65535个ID的倒序的栈，每次获取ID时，从栈顶取，
当ID用完时放回栈中。
同样接收65535个ID上限的限制，这个不变。
但这样做需要预先分配空间。

另一种想法是，利用小根堆来实现重用池，
每次获取ID时，先检查小根堆有无可用ID，如果有就直接用，否则从cur_max_ball_id中自增获取。
当ID用完时，将其放回小根堆中。
但是，小根堆操作获取最小是O(1),但是调整操作是O(logn)，而栈操作全是O(1)，似乎还不如栈。
毕竟，我们不需要保证ID的顺序，只需要保证ID的重用即可。
...
server:
	...
	- ingame //游戏内相关代码
	- model //结构体存放
	- practical_funcs //实用函数库
  main.go //入口函数
  handles.go //处理连接和读写函数

因为目前项目只有一个游戏战局内处理功能，因此连接-多业务层的设计我们放到后面进行。

## 关于客户端的一个奇怪问题
客户端代码当不拖动其窗口时一切正常，而一旦拖动其窗口，立刻无法接受到消息，同时服务端报错写入该连接失败，但不是断开连接。
根据AI的分析，其原因如下：
当拖动 Pygame 窗口时，底层的 SDL 库会进入 C 语言层面的模态消息循环，这个循环会直接冻结 所有 Python 线程（主线程 + 子线程）
目前可能是系统 + 底层库的硬限制。
